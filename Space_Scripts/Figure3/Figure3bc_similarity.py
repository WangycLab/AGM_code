...
mpl.rcParams.update({"pdf.fonttype": 42, "ps.fonttype": 42, "font.family": "Arial", "figure.facecolor": "white", "axes.facecolor": "white", "axes.edgecolor": "black", "axes.linewidth": 0.8, "font.size": 15, "axes.titlesize": 15, "axes.labelsize": 15, "xtick.labelsize": 15, "ytick.labelsize": 15, "legend.fontsize": 15, "lines.linewidth": 2.0, "lines.markersize": 5, "xtick.direction": "out", "ytick.direction": "out"})
...
similarity_matrix = np.array([[1 - jensenshannon(data.iloc[i], data.iloc[j]) for j in range(n)] for i in range(n)])
...
time_colors = {"L-60": "#F4F1DE", "L-30": "#E6D8AD", "FD30": "#5F6F75", "FD90": "#81B295", "FD150": "#A3BFA8", "R+1": "#9A031E", "R+7": "#BA3A26", "R+14": "#E07A5F"}
...
for i, label in enumerate(similarity_df.index):
    ...
    color = time_colors.get(label.split("_")[0], "silver")
    plt.scatter(coords[i, 0], coords[i, 1], c=color, s=500, marker=marker, edgecolor="k", linewidth=0.1)
...
shape_legend = [Line2D([0], [0], marker=m, color="w", label=f"Astronaut {a}", markerfacecolor="gray", markersize=15) for a, m in zip(["1", "2", "3"], ["o", "s", "^"])]
color_legend = [Line2D([0], [0], marker="o", color="w", label=tp, markerfacecolor=col, markersize=15) for tp, col in time_colors.items()]

ax = plt.gca()
legend1 = ax.legend(handles=shape_legend, title="Astronaut", title_fontsize=26, fontsize=23, loc="upper right", bbox_to_anchor=(1.32, 0.99))
ax.add_artist(legend1)
ax.legend(handles=color_legend, title="Timepoint", title_fontsize=26, fontsize=23, loc="upper right", bbox_to_anchor=(1.35, 0.66))
...
ax = sns.heatmap(similarity_df, cmap=cmap_choice, annot=False, square=True, linewidths=0, cbar=True, cbar_kws={"shrink": 0.78, "orientation": "vertical", "pad": 0.03, "aspect": 30})
...
# Add sample and astronaut boundaries
