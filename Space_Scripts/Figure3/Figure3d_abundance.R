...
topN <- 25
top_species_ordered <- df_raw %>% arrange(desc(mean_abundance)) %>% slice(1:topN) %>% pull(Species)
...
make_alluvial <- function(species_df, time_cols, time_labels, species_colors, show_x_axis = TRUE, show_legend = TRUE, subtitle = NULL) {
  df_long <- species_df %>%
    ...
    mutate(alluvium = Species_group, Time = factor(Time, levels = time_cols), Species_group = factor(Species_group, levels = names(species_colors)))
  ...
  if (show_x_axis) {
    p <- p + xlab("Timepoints") + theme(axis.text.x = element_text(size = 12), axis.title.x = element_text(size = 16))
  } else {
...
# Generate astronaut-specific plots
p1 <- make_alluvial(species_df, colnames(df_raw)[1:8], time_labels, species_colors, show_x_axis = FALSE, show_legend = FALSE, subtitle = "Astronaut 1")
p2 <- make_alluvial(species_df, colnames(df_raw)[9:16], time_labels, species_colors, show_x_axis = FALSE, show_legend = FALSE, subtitle = "Astronaut 2")
p3 <- make_alluvial(species_df, colnames(df_raw)[17:24], time_labels, species_colors, show_x_axis = TRUE, show_legend = TRUE, subtitle = "Astronaut 3")

combined_plot <- p1 / p2 / p3 + plot_layout(ncol = 1, guides = "collect")

# Save figure
ggsave("Figure3d.pdf", plot = combined_plot, device = cairo_pdf, width = 7.5, height = 8, units = "in")
