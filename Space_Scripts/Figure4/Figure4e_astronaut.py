...
astro_order = ["Astronaut 1", "Astronaut 2", "Astronaut 3"]

# Load metadata
df = pd.read_csv("cell_metadata_annotated.tsv", sep="\t").dropna(subset=["astronaut", "cluster"])
df["cluster"] = pd.Categorical(df["cluster"].astype(str), categories=cluster_order, ordered=True)
df = df[df["astronaut"].isin(astro_order)]

# Build astronaut × cluster count table
ct_table = pd.crosstab(df["astronaut"], df["cluster"]).reindex(index=astro_order, columns=cluster_order, fill_value=0)
...
sns.heatmap(
    ...
    cbar_kws={"label": "Std. Pearson Residual", "shrink": 0.5, "aspect": 15, "pad": 0.05}
)

# Add significance marks
for i, j in zip(*np.where(annot_mat_T == "*")):
    ax.text(j + 0.5, i + 0.75, "*", fontsize=25, ha="center", va="center", color="white", fontweight="bold")

cbar = ax.collections[0].colorbar
cbar.set_label("Std. Pearson Residual", fontsize=18)
cbar.ax.tick_params(labelsize=15)

ax.set_xlabel("Astronaut", fontsize=25)
ax.set_ylabel("Cluster", fontsize=25)
ax.set_xticks([0.5, 1.5, 2.5])
ax.set_xticklabels(["1", "2", "3"], fontsize=16, rotation=0)
