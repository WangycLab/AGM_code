...
# Load metadata
df = pd.read_csv("cell_metadata_annotated.tsv", sep="\t").dropna(subset=["stage", "cluster"])
df["cluster"] = pd.Categorical(df["cluster"].astype(str), categories=cluster_order, ordered=True)
df = df[df["stage"].isin(stage_order)]

# Build stage × cluster table
ct_table = pd.crosstab(df["stage"], df["cluster"]).reindex(index=stage_order, columns=cluster_order, fill_value=0)
...
ax.set_xlabel("Stage", fontsize=25)
ax.set_ylabel("Cluster", fontsize=25)
ax.set_xticks([0.5, 1.5, 2.5])
ax.set_xticklabels(["L", "FD", "R"], fontsize=16, rotation=0)
ax.set_yticklabels(ax.get_yticklabels(), fontsize=15, rotation=0)

plt.tight_layout()
plt.savefig("Figure4f.pdf", dpi=300, bbox_inches="tight")
plt.show()
